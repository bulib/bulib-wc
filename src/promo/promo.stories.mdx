...
- an `<img>` tag dropped inside, will be used to set the `background-image` of the `.main` section via the `src` attribute
...
<Preview withToolbar>
  <Story name="no-top">
    {html`
      <bulib-promo>
        <img src="https://via.placeholder.com/500x350?text=this+is+a+custom+'<img>'">
        <div slot="bottom">
  ...
  <Story name="no-bottom-heading">
    {html`
      <bulib-promo>
        ...
        <img src="https://via.placeholder.com/750x300?text=this+is+a+different+custom+img">
      </bulib-promo>
    `}
  </Story>
</Preview>
...
<Preview withToolbar>
  <Story name="unconventional-content">
    {html`
      <div class="flexnw flex-around">
        <bulib-promo>
